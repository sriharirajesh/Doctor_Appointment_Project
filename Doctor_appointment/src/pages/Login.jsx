...
const Login = () => {
  ...
  return (
    <form className='min-h-[100vh] w-full flex items-center'>
       <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[440px] sm:min-w-[96px] border rounded-xl text-zinc-600 text-sm shadow-lg'>
        ...
        <p>please{login === 'Sign up' ? 'Sign up' : 'Login'} To Book Appointment </p>
        {
            ...
            <p>Full Name</p>
            <input className='border border-zinc-300 rounded w-full p-2 mt-1 text-black' type='text' onChange={(e)=> setName(e.target.name)} value={name} required />
        </div>
        }
        <div className='w-full'>
            <p>Email</p>
            <input className='border border-zinc-300 rounded w-full p-2 mt-1 text-black' type='email' onChange={(e)=> setEmail(e.target.email)} value={email} required />
        </div>
        <div className='w-full'>
            <p>Password</p>
            <input className='border border-zinc-300 rounded w-full p-2 mt-1 text-black' type ='password' onChange ={(e)=> setPassword(e.target.password)} value={password} required />
        </div>
